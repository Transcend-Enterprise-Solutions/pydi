...
use App\Models\MonthlyWaterBills;
use App\Models\User;
use Carbon\Carbon;
use Exception;
...
class WaterBills extends Component
{
    ...
    public function render()
    {
        $homeowners = User::select([
                ...
                'positions.position',
                'user_data.*',
                'monthly_water_bills.status as payment_status',
                'monthly_water_bills.date_paid',
                'monthly_water_bills.payment_mode'
            ])
            ->join('user_data', 'user_data.user_id', 'users.id')
            ->where('users.user_role', 'homeowner')
            ->leftJoin('positions', 'positions.id', 'users.position_id')
            ->leftJoin('monthly_water_bills', function($join) {
                $join->on('monthly_water_bills.user_id', '=', 'users.id')
                ->where('monthly_water_bills.year', '=', Carbon::parse($this->month)->year)
                ->where('monthly_water_bills.month', '=', Carbon::parse($this->month)->month);
            })
            ...
            ->when($this->paymentStatus !== 'all', function ($query) {
                if ($this->paymentStatus === 'paid') {
                    return $query->whereNotNull('monthly_water_bills.status')
                               ->where('monthly_water_bills.status', '1');
                } else {
                    return $query->where(function($query) {
                        $query->whereNull('monthly_water_bills.status')
                              ->orWhere('monthly_water_bills.status', '!=', '1');
                    });
                }
            })
    ...
    public function toggleEditPayment($id){
        ...
        $monthlyAssocDue = MonthlyWaterBills::where('user_id', $this->userId)
    ...
    public function exportPayments(){
        $query = User::select([
                ...
                'monthly_water_bills.date_paid',
                'monthly_water_bills.payment_mode',
                'monthly_water_bills.reference_number',
            ])
            ->join('user_data', 'user_data.user_id', 'users.id')
            ->where('users.user_role', 'homeowner')
            ->leftJoin('positions', 'positions.id', 'users.position_id')
        ...
        $filters = [
            'query' => $query,
            'title' => 'Water Bill for the Month of ' . Carbon::parse($this->month)->format('F Y'),
        ];
